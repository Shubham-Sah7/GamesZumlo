...
export function HoneydewMascot({ 
  size = 120, 
  mood = 'idle',
  onTap 
}: HoneydewMascotProps) {
  ...
  // Blinking animation
  useEffect(() => {
    if (mood === 'sleeping') return // No blinking when sleeping
    
    let closeTimer: ReturnType<typeof setTimeout>
    let openTimer: ReturnType<typeof setTimeout>

    const schedule = () => {
      // Random delay between blinks: 2–5 s
      closeTimer = setTimeout(() => {
        setBlink(true)
        // Eyes stay closed for 130 ms
  ...
  const getAnimation = () => {
    if (bounce) return 'zummie-bounce 0.8s ease-out'
    if (wiggle) return 'zummie-wiggle 0.5s ease-in-out'
    
    switch (mood) {
      case 'celebrating':
        return 'zummie-celebrate 0.6s ease-in-out infinite'
      case 'thinking':
        return 'zummie-think 3s ease-in-out infinite'
      case 'sleeping':
        return 'zummie-sleep 2s ease-in-out infinite'
      case 'happy':
        return 'zummie-happy 2.5s ease-in-out infinite'
      default:
        return 'zummie-float 2.8s ease-in-out infinite'
    }
  }

  // Eye state based on mood
  const getEyeScale = () => {
    if (blink) return 'scaleY(0.04)'
    if (mood === 'sleeping') return 'scaleY(0.08)'
    if (mood === 'happy' || mood === 'celebrating') return 'scaleY(1.1)'
    return 'scaleY(1)'
  }

  // Pupil position for expressions
  const getPupilTransform = () => {
    if (mood === 'thinking') return 'translate(3px, -2px)'
    if (mood === 'happy' || mood === 'celebrating') return 'translate(0, 2px)'
  ...
  return (
    ...
    >
      <style>{`
        @keyframes zummie-float {
          0%, 100% { transform: translateY(0px);  }
          50%       { transform: translateY(-6px); }
        }
        
        @keyframes zummie-bounce {
        ...
        @keyframes zummie-wiggle {
          0%, 100% { transform: rotate(0deg); }
          25%      { transform: rotate(-8deg); }
          75%      { transform: rotate(8deg); }
        }
        
        @keyframes zummie-celebrate {
          0%, 100% { transform: translateY(0px) scale(1); }
          50%      { transform: translateY(-8px) scale(1.05); }
        }
        
        @keyframes zummie-think {
          0%, 100% { transform: translateY(0px) rotate(0deg); }
          50%      { transform: translateY(-4px) rotate(3deg); }
        }
        
        @keyframes zummie-sleep {
          0%, 100% { transform: translateY(0px); }
          50%      { transform: translateY(3px); }
        }
        
        @keyframes zummie-happy {
          0%, 100% { transform: translateY(0px) scale(1); }
          50%      { transform: translateY(-8px) scale(1.02); }
        }
        
        @keyframes zummie-breathe {
      ...
      <g clipPath="url(#zmcl)">
        {/* Body with subtle breathing */}
        <g style={{ 
          transformOrigin: '84px 84px',
          animation: mood === 'idle' ? 'zummie-breathe 4s ease-in-out infinite' : 'none'
        }}>
          <path
          ...
          {/* Eyes — blink by scaling vertically around their centre (≈ y 88) */}
          <g
            style={{
              transformOrigin: '84px 88px',
              transform: getEyeScale(),
              transition: blink
          ...
          >
            ...
            <g style={{ transform: getPupilTransform(), transition: 'transform 0.3s ease' }}>
              {/* Right pupil */}
              <path
