// src/components/HoneydewMascotEnhanced.tsx
// Enhanced version with moods and animations for React Native
...
export default function HoneydewMascotEnhanced({
  size = 120,
  mood = 'idle',
  onTap,
}: HoneydewMascotProps) {
  ...
  // Breathing animation (for idle mood)
  useEffect(() => {
    if (mood !== 'idle') return;
    ...
    const blinkAnimation = () => {
      ...
      const timer = setTimeout(() => {
        Animated.sequence([
          ...
          Animated.timing(blink, {
            toValue: mood === 'happy' || mood === 'celebrating' ? 1.1 : 1,
            duration: 100,
            useNativeDriver: true,
          }),
        ]).start(() => {
  ...
  const getPupilOffset = () => {
    if (mood === 'thinking') return {x: 3, y: -2};
    if (mood === 'happy' || mood === 'celebrating') return {x: 0, y: 2};
    return {x: 0, y: 0};
  };
  ...
  return (
    <Pressable onPress={onTap ? handleTap : undefined}>
      <Animated.View
        style={{
          width: size,
          height: size,
          transform: [
            {translateY: mood === 'celebrating' ? bounce : float},
        ...
        <Svg width={size} height={size} viewBox="0 0 168 169" fill="none">
          <Defs>
            <ClipPath id="zmcl">
